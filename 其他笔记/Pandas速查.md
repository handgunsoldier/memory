...
pd.DataFrame(lst)
# 指定行列
dates = pd.date_range('20180101', periods=6)  # 创建一个长度为6的时间序列, 单位天
...
"""多重索引情况下索引"""
df.loc[('state', 'city'), :]  # 一级索引, 二级索引
df.loc[('state', 'city')]  # 上面的省略写法
df.loc['state', 'city']  # 上面的省略写法

df.loc[('state', 'city'), ['name']]  # 同时指定列
